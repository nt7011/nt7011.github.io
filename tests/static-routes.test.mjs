...
test("translator version metadata matches the approved version folder", async () => {
  ...
  const betaInstallerCore = await readSiteFile("translator", "4.0.0b4", "installer-core.mjs");

  assert.equal(availableVersions.recommended, "3.2.10");
  assert.equal(availableVersions["recommended-beta"], "4.0.0b4");
  assert.deepEqual(availableVersions.stable, ["3.2.10"]);
  assert.deepEqual(availableVersions.prerelease, ["4.0.0b4"]);
  assert.deepEqual({
